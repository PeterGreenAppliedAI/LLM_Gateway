...
async def get_security_results(
    ...
    Args:
        ...
        guard_only: Only return results where guard model ran
        disagreements_only: Only return results where regex and guard disagree

    Returns:
        List of analysis results with side-by-side verdicts
    """
    if security_analyzer is None:
        return SecurityResultsResponse(results=[], total=0)

    # Get more results than limit to account for filtering
    ...
    for r in raw_results:
        ...
        if disagreements_only:
            ...
            # Disagreement: regex flags but guard says safe, or guard flags but regex missed
